...
public sealed record ImportarNotaEntradaRequest(Guid EmpresaId, Guid DepositoId, string ChaveAcesso, IReadOnlyCollection<ItemNotaEntradaExternaRequest> ItensExternos, IReadOnlyDictionary<string, Guid> Conciliacoes);
public sealed record ConsultarImportacoesNotaEntradaRequest(Guid? EmpresaId, Guid? DepositoId, bool? ImportadaComSucesso, string? ChaveAcesso, int Page = 1, int PageSize = 20);
...
public sealed record ImportacaoNotaEntradaResponse(Guid EmpresaId, Guid DepositoId, string ChaveAcesso, bool ImportadaComSucesso, int ItensExternos, int ItensPendentesConciliacao, int MovimentosGerados, DateTimeOffset ProcessadaEm);
